...
export class NotificationService {
  ...
  dispatchContactCreated(contact: Contact): void {
    setImmediate(() => {
      void this.handleContactCreated(contact).catch((error) => {
        this.logger.error(
          `询盘通知派发失败 contactId=${contact.id}: ${(error as Error).message}`,
        );
      });
    });
  }
  ...
  private async handleContactCreated(contact: Contact): Promise<void> {
    ...
    if (
      notificationSettings.contact.autoReplyEnabled &&
      contact.email?.trim()
    ) {
      ...
      jobs.push({
        ...
        subject:
          notificationSettings.contact.autoReplySubject?.trim() ||
          DEFAULT_AUTO_REPLY_SUBJECT,
  ...
  private async sendEmailJob(job: SendEmailJob): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `邮件发送失败 template=${job.template} to=${job.recipient}: ${message}`,
      );
    }
  }
