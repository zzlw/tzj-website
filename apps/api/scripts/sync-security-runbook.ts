/**
 * 同步 docs/security/2fa-emergency-runbook.md 到文档中心（个人空间）。
 *
 * 背景：admin 文档中心当前仅有「我的文档」个人空间入口（mine=1），
 * 组织共享文档在界面上无法查看，因此同步到指定用户的个人空间。
 *
 * - 目录：该用户个人顶级目录「安全与运维」（slug: security-ops，不存在则创建）
 * - 文档：slug 固定为 2fa-emergency-runbook，重复执行按仓库文件内容覆盖更新（同步语义）
 * - 状态：published
 *
 * 用法：
 *   pnpm --filter @tzj/api run sync:security-runbook                     # 默认同步到超级管理员
 *   pnpm --filter @tzj/api run sync:security-runbook -- --user <登录邮箱>  # 指定其他用户
 */
...
const SOURCE_FILE = join(__dirname, '../../../docs/security/2fa-emergency-runbook.md');
const FOLDER_NAME = '安全与运维';
const FOLDER_SLUG = 'security-ops';
const DOC_SLUG = '2fa-emergency-runbook';
const DOC_TITLE = '2FA 应急处置 Runbook';
const DOC_TAGS = ['安全', '运维手册'];
