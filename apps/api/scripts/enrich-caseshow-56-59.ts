/**
 * 按最新 docs/case-center-content-ai-enrichment-plan.md 重做 caseshow-56-59（模板案例）。
 * - 旧图：黄攀岩墙+白钢架（封面/图0/图1）✅ 作参考；蓝白多层训练塔 ❌ 作废
 * - 按方案「无需脱敏」保留江苏食品药品职业技术学院口径；清除公司模板文案与电话
 * - detailCover：远景/航拍 + 顶部留白（导航安全区）
 *
 * 用法：pnpm --filter @tzj/api exec node --env-file=../../.env ./node_modules/tsx/dist/cli.mjs scripts/enrich-caseshow-56-59.ts
 */
...
const GALLERY = [
  'content/case-caseshow-56-59-gallery-1.webp',
  'content/case-caseshow-56-59-gallery-2.webp',
  'content/case-caseshow-56-59-gallery-3.webp',
] as const;

const PAYLOAD = {
  summary:
    '为江苏食品药品职业技术学院建设户外训练基地：以黄色攀岩墙与白色钢构平台组合为主体，配套绳索架与低矮通过墙，服务学生拓展与基础攀爬通过训练。',
  seoTitle: '江苏食品药品职业技术学院训练基地｜户外攀岩墙案例',
  seoDesc:
    '江苏食品药品职业技术学院训练基地案例：户外攀岩墙、钢构平台与绳索架组合，了解拓之迹院校拓展设施交付。',
  coverImage: 'content/case-caseshow-56-59-hero.webp',
  detailCoverImage: 'content/case-caseshow-56-59-detail-hero.webp',
  images: [...GALLERY],
  highlights: [
    '黄色攀岩墙覆盖基础攀爬与通过类拓展科目',
    '白色钢构平台与护栏便于教员组织观察',
...
![江苏食品药品职业技术学院训练基地 · 另一外景角度（1/3）](content/case-caseshow-56-59-gallery-1.webp)

布局预留器械前方集结与教员观察空间，便于分组轮换，减少对运动场与校园主路动线的干扰。

![江苏食品药品职业技术学院训练基地 · 墙体与钢架近景（2/3）](content/case-caseshow-56-59-gallery-2.webp)
...
![江苏食品药品职业技术学院训练基地 · 仰视平台视角（3/3）](content/case-caseshow-56-59-gallery-3.webp)
