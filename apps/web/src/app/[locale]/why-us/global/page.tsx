    ...
    <div className="pb-20">
      <section className="relative flex min-h-[620px] items-center justify-center overflow-hidden bg-neutral-900 pt-16 lg:min-h-[720px]">
        ...
        <Container className="rb-on-media relative z-10 flex flex-col items-center py-16 text-center lg:py-24">
          ...
          <p className="mt-6 max-w-2xl text-base leading-relaxed text-white/85 md:text-lg">{t("hero.description")}</p>
        </Container>
      </section>

      <section>
        <Container className="py-16 lg:py-24">
          <SectionHeading eyebrow={t("overviewSection.eyebrow")} title={t("overviewSection.title")} description={t("overviewSection.description")} />
          <div className="mt-10 flex flex-wrap gap-3">
            {regions.map((r) => (
              <span key={r} className="border border-neutral-300 bg-neutral-100 px-4 py-2 text-sm font-bold text-neutral-900">{r}</span>
            ))}
          </div>
        </Container>
      </section>
      ...
      <section>
        <Container className="py-16 lg:py-24">
          <SectionHeading eyebrow={t("sectorsSection.eyebrow")} title={t("sectorsSection.title")} description={t("sectorsSection.description")} />
          <div className="mt-10 flex flex-wrap gap-3">
            {sectors.map((s) => (
              <span key={s} className="border border-neutral-300 bg-white px-4 py-2 text-sm font-bold text-neutral-900">{s}</span>
            ))}
          </div>
        </Container>
      </section>
      ...
      <section className="bg-neutral-100">
        <Container className="py-16 lg:py-24">
          <SectionHeading eyebrow={t("highlightsSection.eyebrow")} title={t("highlightsSection.title")} />
          ...
          <blockquote className="mt-10 border-l-4 border-primary bg-white p-8">
            ...
            <footer className="mt-4 text-sm font-bold text-secondary-text">{t("quote.attribution")}</footer>
          </blockquote>
        </Container>
      </section>
