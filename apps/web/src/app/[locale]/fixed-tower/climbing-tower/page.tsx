...
import { Building2, Target, ArrowUpFromLine, Shield, Users, Wrench } from "lucide-react";
...
const IMAGE = "/media/tower-hamilton.jpg";
const FEATURE_ICONS = [Building2, ArrowUpFromLine, Target, Shield, Users, Wrench] as const;
const RELATED_HREFS = ["/accessories/tactical", "/fixed-tower/custom", "/solutions/police"];

export async function generateMetadata() {
  return createPageMetadata({ namespace: "pages.fixedTowerClimbingTower", path: "/fixed-tower/climbing-tower" });
}

export default async function ClimbingTowerPage() {
  const t = await getTranslations("pages.fixedTowerClimbingTower");
      ...
      <JsonLd
        data={[
          breadcrumbJsonLd([
            ...
            { name: t("breadcrumb.current"), path: "/fixed-tower/climbing-tower" },
          ]),
          productJsonLd({
            name: t("jsonLd.productName"),
            description: t("jsonLd.productDescription"),
            path: "/fixed-tower/climbing-tower",
            image: IMAGE,
          }),
        ]}
      />
