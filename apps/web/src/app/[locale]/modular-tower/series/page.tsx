...
import { Container, PageHero, SectionHeading, RbButton, RbLink } from "@/components/ui";
...
const SERIES_IMAGES = ["/media/modular-m.jpg", "/media/modular-o.png", "/media/modular-d.png", "/media/modular-x.png"] as const;
const RELATED_HREFS = ["/modular-tower/vs-containers", "/fixed-tower", "/resources/design-center"];

export async function generateMetadata() {
  return createPageMetadata({ namespace: "pages.modularTowerSeries", path: "/modular-tower/series" });
}
...
export default async function ModularSeriesPage() {
  ...
  const series = t.raw("series") as Array<{ code: string; name: string; desc: string; use: string }>;
    ...
    <div className="pb-20">
            ...
            {series.map((s, i) => (
              <div key={s.code} className="flex flex-col border border-neutral-300 bg-white">
                <div className="relative aspect-[16/10] overflow-hidden border-b border-neutral-300 bg-neutral-100">
                  <Image src={SERIES_IMAGES[i]!} alt={`${s.code} ${s.name}`} fill sizes="(max-width: 640px) 100vw, 50vw" className="object-contain p-4" />
                </div>
                <div className="p-6">
                  <span className="text-xs font-bold uppercase tracking-wide text-primary">{s.code}</span>
      ...
      <section className="bg-neutral-100">
        <Container className="py-16 lg:py-24">
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {advantages.map((a) => (
              <div key={a} className="border-l-2 border-primary bg-white p-5 text-base text-neutral-900">
                {a}
              </div>
            ))}
          </div>
