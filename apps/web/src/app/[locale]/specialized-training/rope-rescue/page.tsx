...
import { Mountain, ArrowDownUp, Anchor, Route, LifeBuoy, ShieldCheck } from "lucide-react";
...
const IMAGE = "/media/tower-denver.jpg";
const FEATURE_ICONS = [Mountain, ArrowDownUp, Anchor, LifeBuoy, Route, ShieldCheck] as const;
const RELATED_HREFS = ["/specialized-training/psychological", "/solutions/mine-rescue", "/fixed-tower"];

export async function generateMetadata() {
  return createPageMetadata({ namespace: "pages.specializedTrainingRopeRescue", path: "/specialized-training/rope-rescue" });
}

export default async function RopeRescuePage() {
  const t = await getTranslations("pages.specializedTrainingRopeRescue");
  ...
  return (
    <>
      <JsonLd data={[breadcrumbJsonLd([{ name: tBread("home"), path: "/" }, { name: t("breadcrumb.parent"), path: "/specialized-training" }, { name: t("breadcrumb.current"), path: "/specialized-training/rope-rescue" }]), productJsonLd({ name: t("jsonLd.productName"), description: t("jsonLd.productDescription"), path: "/specialized-training/rope-rescue", image: IMAGE })]} />
      <div className="pb-20">
        ...
        <section>
          <Container className="py-16 lg:py-24">
            <SectionHeading eyebrow={t("programsSection.eyebrow")} title={t("programsSection.title")} />
