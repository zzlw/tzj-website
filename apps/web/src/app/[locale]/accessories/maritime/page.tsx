...
export async function generateMetadata() {
  return createPageMetadata({ namespace: "pages.accessoriesMaritime", path: "/accessories/maritime" });
}
      ...
      <section>
        <Container className="pt-16 lg:pt-24">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {gallery.map((g, i) => (
              <div key={GALLERY_SRCS[i]} className="relative aspect-[4/3] overflow-hidden bg-neutral-900">
                <Image
                  src={GALLERY_SRCS[i]!}
                  alt={g.alt}
                  fill
      ...
      <section>
        <Container className="py-16 lg:py-24">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16">
            ...
            <div>
              <SectionHeading eyebrow={t("configSection.eyebrow")} title={t("configSection.title")} />
              <dl className="mt-8 divide-y divide-neutral-200 border border-neutral-300 bg-white">
                {config.map((c) => (
                  <div key={c.label} className="flex items-center justify-between gap-4 px-5 py-4">
                    ...
                    <dd className="text-right font-display text-sm font-bold text-neutral-900">{c.value}</dd>
                  </div>
                ))}
              </dl>
