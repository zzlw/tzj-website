...
import { Container, SectionHeading, RbButton, RbLink } from "@/components/ui";
...
const DEFAULT_STEPS: ProcessStep[] = [
  { icon: ClipboardList, title: "需求勘察", desc: "深入了解您的场地条件、训练科目、人员规模与预算，明确建设目标。" },
  { icon: PencilRuler, title: "方案设计", desc: "工程师与消防专家共同定制布局、道具与结构，兼顾真实度与合规性。" },
  { icon: Factory, title: "生产制造", desc: "工厂标准化制造，全流程质量管控，确保每个部件经久耐用。" },
  { icon: Truck, title: "现场安装", desc: "专业团队交钥匙安装，从基础到收尾一站式完成。" },
  { icon: GraduationCap, title: "培训交付", desc: "提供使用培训与验收，让队伍第一时间高效上手。" },
  { icon: ShieldCheck, title: "售后维保", desc: "长期维保、配件供应与合规检查，守护您的长期投资。" },
];
  ...
  return (
    <Container className={cn("pt-16 lg:pt-24", className)}>
      <div className="flex flex-col items-center gap-5 border border-neutral-300 bg-white p-10 text-center md:p-14">
        ...
        {description ? (
          <p className="max-w-xl text-secondary-text">{description}</p>
        ) : null}
