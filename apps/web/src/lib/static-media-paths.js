...
function collectSiteStaticMediaPaths() {
  ...
  for (const item of cases_1.caseStudies) add(item.image);
  for (const item of news_1.newsItems) add(item.image);
  for (const item of blog_1.blogPosts) add(item.image);
  for (const line of product_catalog_1.PRODUCT_LINES) add(line.image);
  for (const meta of solutions_1.SOLUTION_META) add(meta.image);
  return [...set].sort();
}
